...
export default function Page() {
    ...
    const { riskList, updateRiskMaster } = useAppStore();
    const [rowSelectionModel, setRowSelectionModel] = useState([]);
    const { updateIsRiskDrawer } = useAppStore();
    const baseURL = '/api/';
    ...
    const getiForm = async () => {
        ...
        try {
            ...
            // console.log('Masters ', response?.data[0]?.risks);
            getData();
        } catch (error) {
    ...
    return (
        <Layout>
            {!uiloading && (
                <>
                    <Breadcrumb page="Risk Logs" project={projectName} section="Raid" />
                    <div className="mt-10">
                        ...
                        {!loading ? (
                            <div ref={captureRef}>
                                {rows !== undefined && (
                                    <>
                                        {viewMode === "Table" && <TableView rows={rows} from="risk" parent="risk" />}
                                        {viewMode === "Split" && <SplitView rows={rows} from="risk" />}
                                        {viewMode === "Card" && <CardView rows={rows} from="risk" params={['key', 'summary', 'status', 'priority', 'action']} />}
                                        {viewMode === "Kanban" && (
                                            <div style={{ maxWidth: '81vw' }} className="overflow-x-auto mt-2">
                                                <div className="mt-5">
                                                    <div className="p-2 rounded-lg">
                                                        <KanbanBoard rows={riskList} steps={steps} from="risk" key={riskList?.length} />
                                                    </div>
                                                    <div className="p-2 border border-slate-300 rounded-lg mt-32">
                                                        <div className="flex justify-center my-3 mt-5 text-lg font-semibold">
                                                            Open / Owned / Escalated Risks
                                                        </div>
                                                        <Kanban rows={rows} steps={isteps} from="irisk" />
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                        {viewMode === "Charts" && <ChartView rows={rows} from="risk" />}
                                    </>
                                )}
                            </div>
                        ) : (
