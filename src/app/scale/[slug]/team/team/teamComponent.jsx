...
export default function TeamComponent() {
    ...
    const { teamList, updateTeamMaster } = useAppStore();
    const [rowSelectionModel, setRowSelectionModel] = useState([]);
    const { updateIsTeamDrawer } = useAppStore();
    const baseURL = '/api/';
    const { projectName, slug, key } = useSlug();
    const { sessionData, userId, updateTeamList } = useAppStore();
    const [viewMode, updateViewMode] = useState('Table');
    ...
    const getiForm = async () => {
        ...
        try {
            ...
            updateTeamMaster(response?.data[0]?.teams);
            getTeam();
        } catch (error) {
                    ...
                    <div>
                        ...
                        {!loading ? (
                            <div>
                                {rows !== undefined && (
                                    <>
                                        {viewMode === "Table" && <TableView rows={rows} from="team" parent="team" />}
                                        {viewMode === "Split" && <SplitView rows={rows} from="team" />}
                                        {viewMode === "Card" && <CardView rows={rows} from="team" params={['teamName', 'projectName', 'createdBy', 'createdAt']} />}
                                    </>
                                )}
                            </div>
                        ) : (
