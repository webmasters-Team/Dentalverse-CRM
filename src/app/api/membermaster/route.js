...
export async function GET(req) {
    ...
    try {
        const masters = await MemberMaster.find({ organizationId: session.data.organizationId });
...
export async function POST(request) {
    ...
    const masters = await MemberMaster.create(masterData);
...
export async function PUT(request) {
    ...
    const updatedMaster = await MemberMaster.findOneAndUpdate({ organizationId: session.data.organizationId }, updateData, {
        new: true,
        runValidators: true,
    });
