...
export default function DependencyAutocomplete({ handleSeletedRecord }) {
    const baseURL = '/api/';
    ...
    const getData = async () => {
        let posturl = baseURL + `dependency?slug=${slug}`;
        await axios
            .get(posturl)
            .then((res) => {
                setMembers(res.data);
        ...
        <div>
            {!loading && (
                <Autocomplete
                    ...
                    style={{ minWidth: 550 }}
                    renderInput={(params) => (
                        <TextField {...params} label="Dependency" placeholder="Dependency" fontSize="small" size="small" />
                    )}
                />
            )}
        </div>
