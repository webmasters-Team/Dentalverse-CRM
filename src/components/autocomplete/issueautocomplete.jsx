...
export default function IssueAutocomplete({ handleSeletedRecord }) {
    const baseURL = '/api/';
    ...
    const getData = async () => {
        let posturl = baseURL + `issue?slug=${slug}`;
        await axios
            .get(posturl)
            .then((res) => {
                setMembers(res.data);
        ...
        <div>
            {!loading && (
                <Autocomplete
                    ...
                    style={{ minWidth: 550 }}
                    renderInput={(params) => (
                        <TextField {...params} label="Issue" placeholder="Issue" fontSize="small" size="small" />
                    )}
                />
            )}
        </div>
