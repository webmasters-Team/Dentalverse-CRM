...
export default function CreateComment() {
    const baseURL = '/api/';
    const { updateIsCommentDrawer, isCommentDrawer } = useAppStore();
    ...
    const getData = async () => {
        let posturl = baseURL + "backlog?id=" + isCommentDrawer;
        await axios
            .get(posturl)
            .then((res) => {
            ...
            <div className="rounded overflow-hidden">
                <div style={{ maxHeight: '100vh', overflowY: 'auto' }}>
                    <div className="sticky top-0 bg-slate-100 z-10">
                        <div>
                            <IconButton onClick={() => { updateIsCommentDrawer(false) }} edge="start" color="inherit" aria-label="close">
                                <CloseRoundedIcon className="ml-4" />
                            </IconButton>
                        </div>
                        <div className="flex justify-center text-lg font-semibold -mt-5 capitalize">
                            {rows?.summary ? rows?.summary : 'Comments'}
                        </div>
                        <Divider />
                    </div>
                    <div className="mt-4 min-h-[86vh]">
                        {!isLoading ? (
                            <div>
                    ...
                    <div className="sticky bottom-0 bg-slate-100 z-10 min-h-[60px]">
                        ...
                        <div className="mt-3 mb-3">
                            <div className="flex justify-end mr-4">
                                <div>
                                    <div className="flex">
                                        <button onClick={() => { updateIsCommentDrawer(false) }}
