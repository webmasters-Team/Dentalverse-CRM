...
const CreateEvent = () => {
    const baseURL = '/api/';
    const { updateIsTimerDrawer, sessionData, userId, updateTimerList, isTimerDrawer, isClone } = useAppStore();
    ...
    useEffect(() => {
        if (isTimerDrawer !== true && isTimerDrawer !== false) {
            getData();
        }
    }, [isTimerDrawer]);

    const getData = async () => {
        let posturl = baseURL + "timer?id=" + isTimerDrawer;
        await axios
            .get(posturl)
            .then((res) => {
                setRows(res?.data[0]);
    ...
    const getMasterData = async () => {
        ...
        await axios
            .get(posturl)
            .then((res) => {
                setLocalForm(res?.data[0]?.timers);
                setLoading(false);
            })
            .catch((err) => {
                console.log(err);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const successMessage = rows ? 'Timer modified successfully!' : 'Timer added successfully!';
                ...
                <div style={{ maxHeight: '100vh', overflowY: 'auto' }}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="sticky top-0 bg-slate-100 z-10">
                            <div>
                                <IconButton onClick={() => { updateIsTimerDrawer(false) }} edge="start" color="inherit" aria-label="close">
                        ...
                        <div className="sticky bottom-0 bg-slate-100 z-10 min-h-[60px]">
                            ...
                            <div className="mt-3 mb-3">
                                <div className="flex justify-end mr-4">
                                    <div>
                                        <div className="flex">
                                            <button onClick={() => updateIsTimerDrawer(false)}
