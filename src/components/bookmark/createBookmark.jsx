...
import LabelAutocomplete from "./autocomplete";

const schema = yup.object().shape({
    bookmarkName: yup.string().required('Please enter bookmark name.'),
    bookmark: yup.string().url('Please enter a valid URL.').required('Please enter bookmark URL.'),
});

const CreateEvent = () => {
    const baseURL = '/api/';
    const { updateIsBookmarkDrawer, sessionData, userId, updateBookmarkList, isBookmarkDrawer, isClone, labelList } = useAppStore();
    ...
    useEffect(() => {
        if (isBookmarkDrawer !== true && isBookmarkDrawer !== false) {
            getData();
        }
    }, [isBookmarkDrawer]);

    const getData = async () => {
        let posturl = baseURL + "bookmark?id=" + isBookmarkDrawer;
        await axios
            .get(posturl)
            .then((res) => {
                setRows(res?.data[0]);
    ...
    const getMasterData = async () => {
        ...
        await axios
            .get(posturl)
            .then((res) => {
                setLocalForm(res?.data[0]?.bookmarks);
                setLoading(false);
            })
            .catch((err) => {
                console.log(err);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const successMessage = rows ? 'Bookmark modified successfully!' : 'Bookmark added successfully!';
                    ...
                    return (
                        <div className="mb-5" key={field.name}>
                            ...
                            <div className="flex mt-2">
                                <LabelAutocomplete handleSeletedRecord={handleSeletedRecord} />
                            </div>
                ...
                <div style={{ maxHeight: '100vh', overflowY: 'auto' }}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="sticky top-0 bg-slate-100 z-10">
                            <div>
                                <IconButton onClick={() => { updateIsBookmarkDrawer(false) }} edge="start" color="inherit" aria-label="close">
                                    <CloseRoundedIcon className="ml-4" />
                                </IconButton>
                            </div>
                            <div className="flex justify-center text-lg font-semibold -mt-5">
                                {isClone ? 'Clone Bookmark' : (
                                    <span>
                                        {rows ? 'Edit Bookmark' : 'Add Bookmark'}
                                    </span>
                                )}
                            </div>
                            <Divider />
                        ...
                        <div className="sticky bottom-0 bg-slate-100 z-10 min-h-[60px]">
                            ...
                            <div className="mt-3 mb-3">
                                <div className="flex justify-end mr-4">
                                    <div>
                                        <div className="flex">
                                            <button onClick={() => updateIsBookmarkDrawer(false)}
