...
const schema = yup.object().shape({
    ...
    weekEndDate: yup
        .date()
        .min(yup.ref('weekStartDate'), 'Due Date should be greater than or equal to Start Date'),
});

const CreateEvent = () => {
    const baseURL = '/api/';
    const { updateIsTimesheetDrawer, sessionData, userId, updateTimesheetList, isTimesheetDrawer, isClone } = useAppStore();
    ...
    useEffect(() => {
        if (isTimesheetDrawer !== true && isTimesheetDrawer !== false) {
            getData();
        }
    }, [isTimesheetDrawer]);

    const getData = async () => {
        let posturl = baseURL + "timesheet?id=" + isTimesheetDrawer;
        await axios
            .get(posturl)
            .then((res) => {
                // console.log('Timesheet ', res?.data[0]);
                setRows(res?.data[0]);
            })
            .catch((err) => {
                console.log(err);
    ...
    const getMasterData = async () => {
        let posturl = baseURL + "timesheetmaster";
        await axios
            .get(posturl)
            .then((res) => {
                setLocalForm(res?.data[0]?.timesheets);
                getMembers();
            })
            .catch((err) => {
                console.log(err);
    ...
    const onSubmit = async (data) => {
        ...
        if (!data.assignee) {
            Object.assign(data, { assignee: sessionData.data.email });
        }
        try {
            const endpoint = baseURL + `timesheet?slug=${slug}`;
            let method;
            ...
            if (rows) {
                if (isClone) {
                    ...
                    Object.assign(data, { userId: userId });
                    Object.assign(data, { key: currentKey + "-TIME-" });
                    method = "post";
                    const { _id, ...newData } = data;
                    const { data: responseData } = await axios[method](endpoint, newData);
                    updateTimesheetList(responseData);
                } else {
                    Object.assign(data, { createdBy: sessionData.data.fullName });
                    Object.assign(data, { userId: userId });
            ...
            const successMessage = rows ? 'Timesheet modified successfully!' : 'Timesheet added successfully!';
        ...
        if (field.quickAdd) {
            switch (field.type) {
                ...
                case 'team':
                    return (
                        <div>
                            {sessionData.data.role === "Administrator" &&
                                (
                                    <div className="mb-5" key={field.name}>
                                        <label htmlFor={field.name} className="scaleformlabel">
                                            {field.label}
                ...
                <div style={{ maxHeight: '100vh', overflowY: 'auto' }}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="sticky top-0 bg-slate-100 z-10">
                            <div>
                                <IconButton onClick={() => { updateIsTimesheetDrawer(false) }} edge="start" color="inherit" aria-label="close">
                                    <CloseRoundedIcon className="ml-4" />
                                </IconButton>
                            </div>
                            <div className="flex justify-center text-lg font-semibold -mt-5">
                                {isClone ? 'Clone Timesheet' : (
                                    <span>
                                        {rows ? 'Edit Timesheet' : 'Add Timesheet'}
                                    </span>
                                )}
                            </div>
                            <Divider />
                        ...
                        <div className="sticky bottom-0 bg-slate-100 z-10 min-h-[60px]">
                            ...
                            <div className="mt-3 mb-3">
                                <div className="flex justify-end mr-4">
                                    <div>
                                        <div className="flex">
                                            <button onClick={() => updateIsTimesheetDrawer(false)}
