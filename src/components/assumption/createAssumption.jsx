...
const schema = yup.object().shape({
    ...
    assignee: yup.string().required('Please select assumption owner.'),
    reportedDate: yup.date().required('Please enter reported date.'),
});

const CreateEvent = () => {
    const baseURL = '/api/';
    const { updateIsAssumptionDrawer, sessionData, userId, updateAssumptionList, updateIsBacklogUpdated, isBacklogUpdated, isAssumptionDrawer, isClone } = useAppStore();
    ...
    const getriskworkitem = async () => {
        let posturl = baseURL + `riskworkitem?slug=${slug}&assumptionId=${isAssumptionDrawer}`;
        await axios
            .get(posturl)
            .then((res) => {
    ...
    useEffect(() => {
        if (isAssumptionDrawer !== true && isAssumptionDrawer !== false) {
            getData();
        }
    }, [isAssumptionDrawer]);

    const getData = async () => {
        let posturl = baseURL + "assumption?id=" + isAssumptionDrawer;
        await axios
            .get(posturl)
            .then((res) => {
                // console.log('assumption ', res?.data[0]);
                setRows(res?.data[0]);
                getriskworkitem();
            })
            .catch((err) => {
    ...
    const getMasterData = async () => {
        ...
        await axios
            .get(posturl)
            .then((res) => {
                setLocalForm(res?.data[0]?.assumptions);
                getmembers();
            })
            .catch((err) => {
                console.log(err);
    ...
    const onSubmit = async (data) => {
        ...
        if (data.owner) {
            let fullName = getFirstNameByEmail(data.owner);
            Object.assign(data, { fullName: fullName });
        }

        try {
            const endpoint = baseURL + `assumption?slug=${slug}`;
            let method;
            ...
            if (rows) {
                if (isClone) {
                    ...
                    Object.assign(data, { userId: userId });
                    Object.assign(data, { key: currentKey + "-ASSU-" });
                    Object.assign(data, { userId: userId });
                    method = "post";
            ...
            const successMessage = rows ? 'Assumption modified successfully!' : 'Assumption added successfully!';
                ...
                <div style={{ maxHeight: '100vh', overflowY: 'auto' }}>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="sticky top-0 bg-slate-100 z-10">
                            <div>
                                <IconButton onClick={() => { updateIsAssumptionDrawer(false) }} edge="start" color="inherit" aria-label="close">
                                    <CloseRoundedIcon className="ml-4" />
                                </IconButton>
                            </div>
                            <div className="flex justify-center text-lg font-semibold -mt-5">
                                {isClone ? 'Clone Assumption  Log' : (
                                    <span>
                                        {rows ? 'Edit Assumption  Log' : 'Add Assumption  Log'}
                                    </span>
                                )}
                            </div>
                            <Divider />
                        ...
                        <div className="sticky bottom-0 bg-slate-100 z-10 min-h-[60px]">
                            ...
                            <div className="mt-3 mb-3">
                                <div className="flex justify-end mr-4">
                                    <div>
                                        <div className="flex">
                                            <button onClick={() => updateIsAssumptionDrawer(false)}
